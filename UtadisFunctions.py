...
df = np.array([[180, 2, 3], [200, 2, 2], [40, 3, 3], [80, 3, 3], [100, 2, 1], [70, 1, 2], [120, 1, 1]])
pref_order = ['NO', '?', 'YES']
groups = ['YES','YES','?','YES','?','NO','NO']
delta = 0.1
g = [[0, 50, 100, 150, 200], [1, 2, 3], [1, 2, 3]]
monotonicity = [1, 1, 1]

print(df)
u = get_u_part(df, g, monotonicity)
print(u)

A_ineq, b_ineq = linprog_dis_inequalities(u_values=u, groups=groups, pref_order=pref_order, delta=delta, sigma=0.11)
print(A_ineq)
...
c = linprog_dis_costfunction(A_ineq, u, pref_order)
print(c)
...
mean_utilities = meta_optimization(res, c, A_ineq, b_ineq, A_eq, b_eq,g=g, method='interior-point', epsilon=0.01)

wValues, transitionPoints, errors = x_deconstruction(mean_utilities, g, pref_order)
print(wValues)
